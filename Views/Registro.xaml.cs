...
public partial class Registro : ContentPage
{
    ...
    {
        string fechaSeleccionada = fechaPicker.Date.ToString("d"); // Formato corto de fecha
        string pais = paisPicker.SelectedItem?.ToString();
        string ciudad = cuidadPicker.SelectedItem?.ToString();
        double montoInicial = Convert.ToDouble(montoInicialEntry.Text); // Convertir monto inicial a double
        ...
        if (montoInicial > 0 && montoInicial < 1500)
        {
            ...
            // Convertir pagoMensual2 a string para mostrarlo y pasarlo
            pagoMensualEntry.Text = pagoMensual2.ToString("F2");

            // Convertir montoInicial y pagoMensual2 a string al pasarlos a Resumen
